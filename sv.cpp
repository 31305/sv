...
{{.nsv=1},
	...
	{.vm=dn(170),.nsv=1},
	{.vm=vs({71,44,1,49,31,70,1,58,8,66,39,70,13,56,1,44,5,77}),.nv=94,.nsv=1},
	{.vm=vs({13,66,51,1,44,48,2,47,56,3,68,33,46,1,71,44,1,51,4,47,2,47,56,1,70,8,46,33,68,7,66,4,56,1}),.nv=95,.pv=97,.sv=1},
	{.vm=vs({2,60,58,3,70,4,56,1,49,1,50,1,71,4,62,11,56,3}),.nv=96,.pv=98,.sv=1},
	{.vm=vs({1,47,46,8,70,12,56,1,49,1,50,2,49,3,52,10}),.nv=97,.pv=99,.sv=1},
	{.vm=vs({2,75,9,66,1,47,56,1,71,10,43,17,48,3,47,56,1}),.nv=98,.pv=116},
	{.vm=dn(166),.nsv=1},
	...
	{.vm=vs({53,2,75,3,70,4,43,38,66,51,3,60,61,4,74,1,46,1,66,7}),.nv=110,.rv=1},
...
int rk()
{
	...
	{
		st.ks=!st.ks;
		SDL_SetTextureBlendMode(st.vc,!st.ks?SDL_BLENDMODE_NONE:SDL_ComposeCustomBlendMode(SDL_BLENDFACTOR_ONE,SDL_BLENDFACTOR_ONE,SDL_BLENDOPERATION_REV_SUBTRACT,SDL_BLENDFACTOR_ONE,SDL_BLENDFACTOR_ONE,SDL_BLENDOPERATION_ADD));
	};
