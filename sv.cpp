...
std::vector<vv> ls=
{
	...
	{.vm=vs({43,2,68,8,71,44,1,67,1,75,5,49,1,55,52,43,35,51,4,49,43,5,61,61,2,44,50,9,61,44,43,1,55,51,32,70,39,66,66,1,44,1,71,1,44,10,51,48,4,71,61,8,77}),.nv=4},
};
		...
		while(ssv)
		{
			...
			while(ck)	
			{
				...
				auto vsk=[&pv,&kp,&vss](size_t vs)
				{
					...
					while(!(ls[vk].nv==0||((ls[vk].nv==ls[kp].nv||(pnv(ls[vk].nv)&&vk!=vs))&&ss-vss<10)))
					{
						vk=ls[vk].nv;
						pv.push_back(vk);
					}
				...
				for(size_t vk=0;vk<gv.size();vk++)
				{
					...
					for(int k=0;k<ks;k++)
					{
						...
						if(1&&!dv&&pv0.vv&&pv0.vv!=1&&!pv0.n&&(pv.sv))
						{
							if(k==0)
							{
