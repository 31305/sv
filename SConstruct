...
if js:
    ...
    e.Append(LINKFLAGS=['-sAUDIO_WORKLET=1','-sWASM_WORKERS=1','-sEXPORTED_RUNTIME_METHODS=ccall,stringToNewUTF8','-sWASM=1','-O3','-sUSE_SDL=2','-sUSE_SDL_IMAGE=2','-sUSE_SDL_TTF=2','-s','-pthread','--js-library=./tkps.js','--use-preload-plugins','--preload-file=nstp','--preload-file=kv.ttf','-sPTHREAD_POOL_SIZE=4','-sEXPORTED_FUNCTIONS=_main,_malloc,_free'])
    st=e.Object('st.cpp')
