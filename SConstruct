...
if js:
    ...
    e.Append(CCFLAGS=['-sUSE_SDL=2','-MJs.o.json']+(['-sUSE_SDL_GFX=2'] if False else [])+(['-Icairo/src','-Icairo/tp/src','-DCP'] if cp else []))
    e.Append(LINKFLAGS=['--preload-file=sc.bmp','-sAUDIO_WORKLET=1','-sWASM_WORKERS=1','-sEXPORTED_RUNTIME_METHODS=ccall','-sWASM=1','-O3','-sUSE_SDL=2','-pthread']+(['-sUSE_SDL_GFX=2'] if False else []))
    st=e.Object('st.cpp')
